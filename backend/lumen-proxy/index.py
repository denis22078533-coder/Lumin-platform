...
def parse_openai_stream(raw: str) -> str:
    """Ð¡Ð¾Ð±Ð¸ÑÐ°ÐµÑ Ð¿Ð¾Ð»Ð½ÑÐ¹ ÑÐµÐºÑÑ Ð¸Ð· SSE-ÑÑÑÐ¸Ð¼Ð° OpenAI."""
    result = []
...
def parse_claude_stream(raw: str) -> str:
    """Ð¡Ð¾Ð±Ð¸ÑÐ°ÐµÑ Ð¿Ð¾Ð»Ð½ÑÐ¹ ÑÐµÐºÑÑ Ð¸Ð· SSE-ÑÑÑÐ¸Ð¼Ð° Claude."""
    result = []
    for line in raw.splitlines():
        line = line.strip()
        if not line.startswith("data:"):
...
def handler(event: dict, context) -> dict:
    """ÐÑÐ¾ÐºÑÐ¸ÑÑÐµÑ Ð·Ð°Ð¿ÑÐ¾ÑÑ Ðº OpenAI/Claude API ÑÐ¾ ÑÑÑÐ¸Ð¼Ð¸Ð½Ð³Ð¾Ð¼. api_key Ð¸ base_url Ð±ÐµÑÑÑÑÑ Ð¸Ð· ÑÐµÐ»Ð° Ð·Ð°Ð¿ÑÐ¾ÑÐ°."""
    ...
    # ÐÐºÐ»ÑÑÐ°ÐµÐ¼ ÑÑÑÐ¸Ð¼Ð¸Ð½Ð³
    body["stream"] = True
